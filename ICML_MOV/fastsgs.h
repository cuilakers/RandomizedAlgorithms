...
#ifndef ICML_MOV_FASTSGS_H
#define ICML_MOV_FASTSGS_H

#include "twingreedy.h"
pair<double,long long int> fastsgs(double eps,dataset* data,int m)
{
                ...
                for(int j=0;j<S.size();j++)
                {
                    ...
                    if(flag)
                    {
                        ...
                        if(marginal>=tau)//if f(e|S_i)>=tau
                        {
                            ...
                            for(int i=0;i<S[j].genres_sum.size();i++)//S+e\in I
                            {
                                S[j].genres_sum[i]+=genres_index[element][i];
                            }
                            available[element]=0;
